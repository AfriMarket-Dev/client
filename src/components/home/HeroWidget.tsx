...
export const HeroWidget: React.FC<HeroWidgetProps> = ({
  ...
  return (
    <div className={`bg-white dark:bg-slate-900 border border-border/60 p-6 flex flex-col h-full hover:shadow-lg transition-all duration-500 relative group/widget rounded-lg ${className}`}>
      {/* Decorative Accent */}
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <Link to={href} className="text-muted-foreground hover:text-primary transition-all p-1 group">
            ...
            {item && typeof item === 'object' ? (
              <Link to={`/products/${item.id}`} className="block h-full transition-colors relative">
                ...
                <div className="aspect-square bg-muted/20 overflow-hidden border border-border/40 group-hover/item:border-primary/30 transition-all duration-500 relative mb-2">
                    ...
                    {item.label && (
                        <div className="absolute top-0 left-0 bg-primary text-white text-[9px] font-bold px-2 py-0.5 shadow-sm z-10 uppercase tracking-widest">
                            {item.label}
                        </div>
                    )}
                </div>
                ...
                // Placeholder
              <div className="w-full aspect-square bg-muted/10 border border-dashed border-border/40 flex items-center justify-center mb-3">
                 <div className="w-4 h-4 bg-border/20 rounded-full" />
              </div>
            )}
          </div>
        ))}
