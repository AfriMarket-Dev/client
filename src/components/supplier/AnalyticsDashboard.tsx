...
const AnalyticsDashboard: React.FC = () => {
  ...
  const getMaxValue = () =>
    Math.max(
      ...getCurrentChartData().map((d) => Math.max(d.value, d.secondary)),
    );
        ...
        {stats.map((stat, index) => (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div
                className={`p-3 rounded-xl ${stat.color === "primary" ? "bg-primary/10" : `bg-${stat.color}-100`}`}
              >
                <stat.icon
                  className={`w-6 h-6 ${stat.color === "primary" ? "text-primary" : `text-${stat.color}-600`}`}
                />
              </div>
              <div
          ...
          <div className="flex items-center justify-center space-x-6 mb-6">
            <div className="flex items-center">
              ...
              <span className="text-sm text-gray-600">
                {getChartLabels().primary}
              </span>
            </div>
            <div className="flex items-center">
              <div className="w-4 h-4 bg-green-500 rounded-full mr-2"></div>
              <span className="text-sm text-gray-600">
                {getChartLabels().secondary}
              </span>
            </div>
          </div>
          ...
          <div className="relative w-48 h-48 mx-auto mb-6">
            ...
            >
              {categoryPerformance.map((category, index) => {
                const startAngle = categoryPerformance
                  .slice(0, index)
