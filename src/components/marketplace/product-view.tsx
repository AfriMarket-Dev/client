	...
	if (isLoading) {
		return (
			<div className="flex min-h-screen items-center justify-center bg-background">
				<div className="text-xs uppercase tracking-widest text-muted-foreground">Loading product...</div>
			</div>
		);
	}
			...
			<div className="mx-auto max-w-[1400px] px-4 py-6 md:px-6">
				<div className="mb-4 flex items-center justify-between">
					...
					<Button variant="outline" onClick={handleToggleWishlist}>
						{isInWishlist ? <RiHeartFill className="mr-2 h-4 w-4" /> : <RiHeartLine className="mr-2 h-4 w-4" />}
					...
					<div className="space-y-6 lg:col-span-7">
						<div>
							<div className="mb-3 flex items-center gap-2">
								<Badge variant="outline" className="text-[10px] uppercase tracking-wider">Product</Badge>
								<span className="text-[10px] uppercase tracking-wider text-muted-foreground">Ref {listing.id.slice(0, 8)}</span>
							</div>
							<h1 className="text-3xl font-display font-bold text-foreground">{listing.name}</h1>
							<div className="mt-3 flex items-baseline gap-2">
								<span className="text-2xl font-semibold text-primary">RWF {price.toLocaleString()}</span>
								<span className="text-xs uppercase tracking-wider text-muted-foreground">/ {variant?.unit ?? "unit"}</span>
							</div>
						</div>
						...
						<Tabs defaultValue="overview" className="w-full">
							...
							<TabsContent value="overview" className="mt-6 space-y-6">
								<div>
									<h3 className="mb-2 text-xs uppercase tracking-wider text-muted-foreground">Description</h3>
								...
								<div className="grid grid-cols-2 gap-3">
									{keyFacts.map((fact) => (
										<div key={fact.label} className="rounded-sm border border-border bg-muted/10 p-3">
							...
							<TabsContent value="supplier" className="mt-6">
								<div className="space-y-4 rounded-sm border border-border bg-muted/5 p-4">
									<div className="flex items-start justify-between">
										<div>
											<p className="text-lg font-heading font-bold text-foreground">{company?.name ?? "Unknown supplier"}</p>
											<p className="text-sm text-muted-foreground">{company?.district ?? "-"}</p>
										</div>
										<Button
											variant="outline"
									...
									<div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
										<span className="flex items-center gap-1">
											...
											{company?.isVerified ? "Verified supplier" : "Unverified supplier"}
										</span>
										<span className="flex items-center gap-1"><RiMapPinLine className="h-3.5 w-3.5" /> {company?.address ?? "Address not provided"}</span>
										{company?.phone && (
							...
							<TabsContent value="reviews" className="mt-6">
								<div className="rounded-sm border border-border bg-muted/5 p-5 text-center">
									...
									<p className="text-sm text-muted-foreground">Ratings and written reviews are not available yet.</p>
								</div>
							</TabsContent>
						</Tabs>

						<div className="flex gap-3 border-t border-border pt-4">
							<Button variant="outline" className="h-11 rounded-sm" onClick={() => setMessageOpen(true)}>
								<RiChat3Line className="mr-2 h-4 w-4" /> Message
							</Button>
							<Button className="h-11 rounded-sm" onClick={() => setMessageOpen(true)}>
								Request quote
							</Button>
						</div>
					</div>
			...
			<Dialog open={messageOpen} onOpenChange={setMessageOpen}>
				<DialogContent className="sm:max-w-md rounded-none border-border">
					...
					<p className="mb-4 text-sm text-muted-foreground">
						Send a message about &quot;{listing.name}&quot;. Continue the conversation in Messages.
					</p>
					<Textarea
						value={messageText}
