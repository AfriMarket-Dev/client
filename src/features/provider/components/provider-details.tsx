			...
			<div className="bg-background/80 backdrop-blur-md border-b border-border py-4 px-4 sm:px-8 lg:px-12 sticky top-0 z-30">
				<div className="w-full flex items-center justify-between gap-4">
					...
					<div className="flex items-center gap-4 shrink-0">
						<Badge className="bg-muted/50 text-foreground border-none text-xs font-medium px-3 py-1 rounded-none">
							{isFetching
								? "Syncing..."
								: company.isVerified
									? "Verified Entity"
					...
					<div className="w-full lg:col-span-1">
						<div className="space-y-12">
							...
							<div className="bg-muted/5 border border-border p-8 space-y-8">
								<h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-foreground border-b border-border pb-4">
									Quick Contact
								</h4>
									...
									<div className="flex flex-wrap gap-4">
										{company.website && (
											<a
												...
												className="h-10 w-10 flex items-center justify-center border border-border hover:border-primary hover:text-primary transition-all rounded-none"
												title="Website"
											>
												<RiGlobeLine size={18} />
											</a>
										...
										{company.socialLinks?.facebook && (
											<a
												href={`https://${company.socialLinks.facebook.replace(/^https?:\/\//, "")}`}
												target="_blank"
												rel="noopener noreferrer"
												className="h-10 w-10 flex items-center justify-center border border-border hover:border-primary hover:text-primary transition-all rounded-none"
												title="Facebook"
											>
												<RiFacebookBoxLine size={18} />
											</a>
										)}
										{company.socialLinks?.instagram && (
											<a
												href={`https://instagram.com/${company.socialLinks.instagram.replace(/^@/, "")}`}
												target="_blank"
												rel="noopener noreferrer"
												className="h-10 w-10 flex items-center justify-center border border-border hover:border-primary hover:text-primary transition-all rounded-none"
												title="Instagram"
										...
										{company.socialLinks?.linkedin && (
											<a
												href={`https://${company.socialLinks.linkedin.replace(/^https?:\/\//, "")}`}
												target="_blank"
												rel="noopener noreferrer"
												className="h-10 w-10 flex items-center justify-center border border-border hover:border-primary hover:text-primary transition-all rounded-none"
												title="LinkedIn"
										...
										{company.socialLinks?.twitter && (
											<a
												href={`https://twitter.com/${company.socialLinks.twitter.replace(/^@/, "")}`}
												target="_blank"
												rel="noopener noreferrer"
												className="h-10 w-10 flex items-center justify-center border border-border hover:border-primary hover:text-primary transition-all rounded-none"
												title="Twitter"
										...
										{company.socialLinks?.youtube && (
											<a
												href={`https://${company.socialLinks.youtube.replace(/^https?:\/\//, "")}`}
												target="_blank"
												rel="noopener noreferrer"
												className="h-10 w-10 flex items-center justify-center border border-border hover:border-primary hover:text-primary transition-all rounded-none"
												title="YouTube"
