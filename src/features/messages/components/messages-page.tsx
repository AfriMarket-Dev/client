	...
	const groupedMessages = useMemo(() => {
		...
		messages.forEach((m) => {
			...
			if (
				isSameSender &&
				groups.length > 0 &&
				groups[groups.length - 1].type === "message-group"
			) {
				(
					groups[groups.length - 1] as Extract<
						MessageGroup,
						{ type: "message-group" }
					>
				).messages.push(m);
		...
		<div className="h-[calc(100vh-56px)] flex bg-background overflow-hidden relative">
			...
			>
				{activeConversation ? (
					...
					<div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-muted/30 bg-[radial-gradient(var(--color-border)_1px,transparent_1px)] bg-[size:40px_40px]">
						<Empty className="max-w-xs border-none bg-transparent">
							<EmptyHeader>
								<EmptyMedia className="w-16 h-16 rounded-full bg-background shadow-sm border border-border flex items-center justify-center mb-4">
									<RiChat3Line className="w-8 h-8 text-muted-foreground/20" />
								</EmptyMedia>
