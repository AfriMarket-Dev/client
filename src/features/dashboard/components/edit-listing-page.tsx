import { RiAddLine, RiDeleteBinLine, RiEditLine, RiInformationLine, RiListSettingsLine } from "@remixicon/react";
...
import type { CreateProductVariantInput, ProductVariant } from "@/types";
...
function EditVariantDialog({
	...
	open: boolean;
	onClose: () => void;
	onSubmit: (data: Partial<CreateProductVariantInput>) => void;
	isLoading: boolean;
...
export function ProviderListingEditPage() {
	...
	if (
		...
		return (
			<div className="p-8 text-center">
				<p className="text-muted-foreground font-heading font-black uppercase tracking-widest text-[10px]">Listing not found.</p>
				<Button
					variant="outline"
	...
	if (isLoading || !item) {
		return (
			<div className="p-8 flex items-center justify-center min-h-[400px]">
				<div className="animate-pulse text-muted-foreground font-heading font-black uppercase tracking-widest text-[10px]">Loading...</div>
			</div>
		);
	}
	...
	const handleUpdateProduct = async (values: any) => {
		...
		try {
			const { price: _price, stock: _stock, unit: _unit, categoryId, ...masterInfo } = values;
			...
			await updateProduct({ id: listingId, data: payload as any }).unwrap();
	...
	const handleUpdateService = async (values: any) => {
		...
		try {
			...
			await updateService({ id: listingId, data: sanitizedData as any }).unwrap();
		...
		<div className="p-8 max-w-[1800px] mx-auto">
			<div className="mb-10 text-center md:text-left">
				...
				<p className="text-muted-foreground text-sm font-medium">
					{itemType === "PRODUCT" ? "Manage your product catalog and inventory options." : "Manage your professional service listing."}
				</p>
			</div>
			...
			{itemType === "PRODUCT" && product && (
				<Tabs defaultValue="general" className="w-full">
					...
					<TabsContent value="general" className="mt-0 outline-none animate-in fade-in duration-500">
									...
									<table className="w-full border-collapse">
										<thead>
											<tr className="border-b border-border/40 bg-muted/10">
												<th className="text-left p-4 text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Variant Name</th>
												<th className="text-left p-4 text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Price (RWF)</th>
												<th className="text-left p-4 text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Stock</th>
												<th className="text-right p-4 text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Actions</th>
											</tr>
										</thead>
												...
												<tr key={v.id} className="group hover:bg-muted/5 transition-colors">
													<td className="p-4">
														<div className="flex flex-col">
															<span className="font-bold text-xs uppercase tracking-tight">{v.name}</span>
															<span className="text-[9px] font-mono font-bold text-muted-foreground/40 uppercase tracking-widest">SKU: {v.sku || "N/A"}</span>
														</div>
													</td>
													<td className="p-4">
													...
													<td className="p-4">
														<div className="flex items-center gap-2">
															<span className={`font-bold text-xs ${v.stock > 0 ? 'text-foreground' : 'text-destructive'}`}>
																{v.stock}
															</span>
															<span className="text-[9px] font-black uppercase text-muted-foreground/40">{v.unit || "unit"}</span>
															{v.stock <= 5 && v.stock > 0 && (
																<Badge variant="warning" className="h-4 text-[7px] font-black px-1.5 rounded-none uppercase">Low</Badge>
															)}
														</div>
													</td>
													<td className="p-4">
														<div className="flex justify-end gap-1">
															...
															<Button
																...
																onClick={async () => {
																	if (confirm(`Remove the "${v.name}" option?`)) {
																		try {
				...
				<div className="max-w-2xl bg-card border border-border/50 p-8 shadow-sm rounded-none mx-auto md:mx-0">
					<ServiceForm
						...
						initialValues={{
							...
							specifications: (service.specifications as any) ?? {},
						}}
