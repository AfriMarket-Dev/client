      ...
      <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">
            Track your saved listings and supplier conversations.
          </p>
        </div>
        <div className="flex gap-3">
          <Button
            size="lg"
            className="h-11 rounded-sm font-heading font-bold uppercase tracking-wider"
          >
          ...
          <Button
            size="lg"
            variant="outline"
            className="h-11 w-11 rounded-sm border border-border p-0"
          >
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-3">
        <Card className="rounded-sm border border-border shadow-none">
          <CardContent className="p-5">
            ...
            <p className="mt-2 text-3xl font-heading font-black text-primary">
              {wishlist.length}
            </p>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          ...
          {loadingWishlist ? (
            ...
            <Card className="rounded-sm border border-dashed border-border">
            ...
            wishlist.slice(0, 6).map((listing: any) => (
              <Card
                key={listing.id}
                className="rounded-sm border border-border shadow-none"
              >
                <CardContent className="p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <h3 className="mt-1 text-lg font-heading font-bold text-foreground">
                        {listing.name}
                      </h3>
                      <p className="text-sm text-primary">
                        RWF{" "}
                        {(listing.variants?.[0]?.price ?? 0).toLocaleString()}
                      </p>
                    </div>
                    <Button
                      variant="outline"
        ...
        <div className="space-y-4">
          ...
          <Card className="rounded-sm border border-border bg-muted/30">
            <CardContent className="p-5 text-sm text-muted-foreground">
              ...
              Keep negotiations and delivery details inside platform messages
              for traceability.
            </CardContent>
          </Card>
        </div>
